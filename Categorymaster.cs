...
namespace sms
{
    public partial class Categorymaster : Form
    {
        ...
        {
            LoadGrid();
            dataGridView1.Columns["CAT_ID"].Width = 0;
            dataGridView1.Columns["CAT_ID"].Visible = false;
            dataGridView1.Columns["CAT_DESC"].Width = 200;
        ...
        private void LoadGrid()
        {
            ...
            SqlDataAdapter da = new SqlDataAdapter("select * from CATegory_T where CAT_DESC like '" + txtstatename.Text + "%' ORDER BY CAT_DESC", sc);
       ...
       
        {
            ...
            {
               MyMessageBox.ShowBox("Category Should not be blank...");
            }
            else
            {
                SqlCommand com = new SqlCommand("Insert_Category '" + txtstatename.Text + "'", sc);
                SqlDataReader dr;
                sc.Open();
                try
                {
                    ...
                    {
                       MyMessageBox.ShowBox("Category Created...");
                        Cancil_form();
                        LoadGrid();
                    }
                    else
                    {
                       MyMessageBox.ShowBox("This Category Name already exist");
                    }
        ...
        
        {
            ...
            {
                SqlCommand com = new SqlCommand("update_Category '" + txtstateid.Text + "','" + txtstatename.Text + "'", sc);
                SqlDataReader dr;
                sc.Open();
                try
                {
                    ...
                    if (dr.RecordsAffected > 0)
                    {
                       MyMessageBox.ShowBox("Category Updated...");
                        Cancil_form();
                        LoadGrid();
                    }
                    else
        ...
        {
            DialogResult dialogResult = MessageBox.Show("Are You Sure Delete Category!!!", "Confirm", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
            if (dialogResult == DialogResult.Yes)
            {
                try
                {
                    ...
                    SqlCommand objCmd = new SqlCommand("DELETE FROM category_t WHERE caste_id=" + txtstateid.Text + "", conn);
                    SqlDataReader dr;
                    objCmd.Connection.Open();
                    dr = objCmd.ExecuteReader(CommandBehavior.CloseConnection);
                    if (dr.RecordsAffected > 0)
                    {
                       MyMessageBox.ShowBox(" Category Deleted Successfully!!!");
                        Cancil_form();
                        InitGrid();
                    }
                    else
        ...
        private void dataGridView1_Click(object sender, EventArgs e)
        {
            ...
            {
                txtstatename.Text = dataGridView1.CurrentRow.Cells["CAT_DESC"].Value.ToString();
                txtstateid.Text = dataGridView1.CurrentRow.Cells["cat_id"].Value.ToString();
